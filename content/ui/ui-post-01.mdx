...
This component combines **[ShadCNUI](https://ui.shadcn.com)'s** `useMediaQuery` hook with a `Dialog` and `Drawer` component to create a responsive modal that renders a `Dialog` on desktop and a `Drawer` on mobile.
